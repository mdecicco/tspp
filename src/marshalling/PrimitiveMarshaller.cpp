...
namespace tspp {
    ...
    v8::Local<v8::Value> PrimitiveMarshaller::convertToV8(CallContext& context, void* value, bool valueNeedsCopy) {
        v8::Isolate* isolate = context.getIsolate();
        const bind::type_meta& meta = m_dataType->getInfo();

        if (m_isBoolean) {
            return v8::Boolean::New(isolate, *((bool*)value));
        } else if (meta.is_integral) {
            if (meta.is_unsigned) {
                switch (meta.size) {
        ...
        isolate->ThrowException(
            v8::Exception::TypeError(
                v8::String::NewFromUtf8(isolate, "Unsupported type").ToLocalChecked()
            )
        );
    ...
    void* PrimitiveMarshaller::convertFromV8(CallContext& callCtx, const v8::Local<v8::Value>& value) {
        ...
        const bind::type_meta& meta = m_dataType->getInfo();
        u8* data = nullptr;
        if (meta.size > 0) data = callCtx.alloc(m_dataType);
        else callCtx.setNextAllocation(nullptr);
