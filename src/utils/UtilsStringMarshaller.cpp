...
namespace tspp {
    ...
    v8::Local<v8::Value> UtilsStringMarshaller::convertToV8(CallContext& context, void* value, bool valueNeedsCopy) {
        v8::Isolate* isolate = context.getIsolate();
        const bind::type_meta& meta = m_dataType->getInfo();

        String* str = (String*)value;

        return v8::String::NewFromUtf8(isolate, str->size() > 0 ? str->c_str() : "").ToLocalChecked();
    }
        ...
        if (!value->IsString()) {
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(isolate, "Value is not a string").ToLocalChecked()
                )
            );
