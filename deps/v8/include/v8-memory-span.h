...
class V8_EXPORT MemorySpan {
 ...
 public:
  ...
  class Iterator {
   ...
   private:
    friend class MemorySpan<IT>;

    constexpr explicit Iterator(IT* ptr) : ptr_(ptr) {}

    IT* ptr_ = nullptr;
  };

  [[nodiscard]] Iterator<T> begin() const { return Iterator<T>(data_); }
  [[nodiscard]] Iterator<T> end() const { return Iterator<T>(data_ + size_); }
