...
namespace tspp {
    ...
    void AsyncCallJob::afterComplete() {
        ...
        if (m_hasException) {
            resolver->Reject(
                context,
                v8::Exception::Error(
                    v8::String::NewFromUtf8(m_isolate, m_exceptionMsg.c_str()).ToLocalChecked()
                )
            );
            return;
        }
        ...
        if (retInfo.size > 0) {
            ...
            v8::Local<v8::Value> retVal = data.marshaller->toV8(m_callContext, m_result, retNeedsCopy);
    ...
    void AsyncCallJob::setup(void* selfPtr, const v8::FunctionCallbackInfo<v8::Value>& args) {
        ...
        if (retInfo.size > 0) {
            if (retObjMgr) m_result = retObjMgr->preemptiveAlloc();
            else m_result = m_callContext.alloc(retType);
        }
