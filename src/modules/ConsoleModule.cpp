...
namespace tspp {
    ...
    ConsoleModule::ConsoleModule(ScriptSystem* scriptSystem)
        : IScriptSystemModule(scriptSystem, "", "Console")
    {
    }
    ...
    bool ConsoleModule::initialize() {
        ...
        if (!isolate) {
            logError("Cannot initialize console: Isolate not available");
            return false;
        }
        ...
        m_inspectorSession = m_inspector->connect(
            1,
            this,
            state,
        ...
        // Enable console API
        m_inspectorSession->dispatchProtocolMessage(createStringView(String("{\"id\":1,\"method\":\"Runtime.enable\"}")));
        return true;
    }
