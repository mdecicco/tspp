...
namespace tspp {
    ...
    Callback::Callback(
        v8::Isolate* isolate, void* closure, bind::FunctionType* sig, const v8::Local<v8::Function>& target
    ) {
        m_isolate = isolate;
        m_closure = closure;
        m_sig     = sig;
    ...
    void Callback::AddRef(void* callback) {
        ...
        if (it == s_map.end()) {
            throw InputException("Attempted to add reference to unbound callback");
        }
    ...
    void Callback::Release(void* callback) {
        auto it = s_map.find(callback);
        if (it == s_map.end()) {
            throw InputException("Attempted to release unbound callback");
        }
    ...
    void* Callback::Create(v8::Isolate* isolate, bind::FunctionType* sig, const v8::Local<v8::Function>& target) {
        ...
        if (ffi_prep_closure_loc((ffi_closure*)closure, cif, invokeCallback, cb, fptr) != FFI_OK) {
            m_pool.free(cb);
    ...
    void invokeCallback(ffi_cif* cif, void* ret, void** args, void* user_data) {
        ...
        if (tryCatch.HasCaught()) {
            ...
            if (tryCatch.StackTrace(context).ToLocal(&stackTrace) && !stackTrace.IsEmpty()) {
                v8::Local<v8::String> stackTraceStr;
                if (stackTrace->ToString(context).ToLocal(&stackTraceStr) && !stackTraceStr.IsEmpty()) {
                    v8::String::Utf8Value trace(isolate, stackTraceStr);
                    throw GenericException(utils::String::Format("Caught error in callback: %s\n%s", *error, *trace));
                }
            }

            throw GenericException(utils::String::Format("Caught error in callback: %s", *error));
        }
        ...
        if (retType->getInfo().size > 0) {
            if (result.IsEmpty()) {
                throw GenericException("Callback did not return a value when one was expected");
            }

            v8::Local<v8::Value> retVal;
            if (!result.ToLocal(&retVal)) {
                throw GenericException("Failed to get return value from callback");
            }
