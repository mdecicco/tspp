...
namespace tspp {
    ...
    void* FunctionMarshaller::convertFromV8(CallContext& callCtx, const v8::Local<v8::Value>& value) {
        ...
        if (!value->IsFunction()) {
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(isolate, "Value is not a function").ToLocalChecked()
                )
            );
        ...
        void* fptr = Callback::Create(
            isolate,
            (bind::FunctionType*)m_dataType,
        ...
        if (!fptr) {
            isolate->ThrowException(
                v8::Exception::Error(
                    v8::String::NewFromUtf8(isolate, "Failed to create closure").ToLocalChecked()
                )
            );
