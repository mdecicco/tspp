...
namespace tspp {
    ...
    bool NonTrivialMarshaller::canAccept(
        v8::Isolate* isolate,
        ...
        if (obj->InternalFieldCount() < 3) return false;

        bind::DataType* type = static_cast<bind::DataType*>(
            obj->GetInternalField(1).As<v8::Value>().As<v8::External>()->Value()
        );
        if (type != m_dataType) return false;

        void* data = obj->GetInternalField(0).As<v8::Value>().As<v8::External>()->Value();
        if (data == (void*)intptr_t(0xdeadbeef)) return false;
            ...
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(
                        isolate,
                        String::Format(
                            "Unable to convert object of type '%s' to JavaScript object: Bound type has not been initialized.",
                            m_dataType->getName().c_str()
                        ).c_str()
                    ).ToLocalChecked()
                )
    ...
    void* NonTrivialMarshaller::convertFromV8(
        CallContext& callCtx,
        const v8::Local<v8::Value>& value
    ) {
        v8::Isolate* isolate = callCtx.getIsolate();
        ...
        if (!value->IsObject()) {
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(isolate, "Expected an object").ToLocalChecked()
                )
            );
            return nullptr;
        }
            ...
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(
                        isolate,
                        String::Format(
                            "Provided object is of type '%s', expected '%s'.",
                            type->getName().c_str(),
                            m_dataType->getName().c_str()
                        ).c_str()
                    ).ToLocalChecked()
            ...
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(
                        isolate,
                        String::Format(
                            "Object of type '%s' has been destroyed.",
                            m_dataType->getName().c_str()
                        ).c_str()
                    ).ToLocalChecked()
                )
        ...
        if (callCtx.hasAllocationTarget()) {
            ...
            m_dataType->findConstructors({ (bind::DataType*)m_dataType->getPointerType() }, true, bind::FullAccessRights, &copyCtor);
            if (!copyCtor) {
                isolate->ThrowException(
                    v8::Exception::TypeError(
                        v8::String::NewFromUtf8(
                            isolate,
                            String::Format(
                                "Unable to convert object of type '%s' from JavaScript object: Object needs copy and no copy constructor found.",
                                m_dataType->getName().c_str()
                            ).c_str()
                        ).ToLocalChecked()
                    )
            ...
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(
                        isolate,
                        String::Format(
                            "Unable to convert object of type '%s' to JavaScript object: Object needs copy and no copy constructor found.",
                            m_dataType->getName().c_str()
                        ).c_str()
                    ).ToLocalChecked()
                )
