...
namespace tspp {
    ...
    bool TrivialStructMarshaller::canAccept(v8::Isolate* isolate, const v8::Local<v8::Value>& value) {
        ...
        for (u32 i = 0; i < properties.size(); i++) {
            ...
            if (prop.offset < 0) continue;

            const DataTypeUserData& userData = prop.type->getUserData<DataTypeUserData>();

            v8::MaybeLocal<v8::Value> maybePropValue = obj->Get(
                context,
                v8::String::NewFromUtf8(isolate, prop.name.c_str()).ToLocalChecked()
            );

            if (maybePropValue.IsEmpty()) continue;

            v8::Local<v8::Value> propValue = maybePropValue.ToLocalChecked();
            if (propValue->IsUndefined() || propValue->IsNull()) continue;

            if (!userData.marshallingData->canAccept(isolate, propValue)) return false;
        }

        return true;
    }

    v8::Local<v8::Value> TrivialStructMarshaller::convertToV8(CallContext& callCtx, void* value, bool valueNeedsCopy) {
        ...
        for (u32 i = 0; i < properties.size(); i++) {
            ...
            obj->Set(
                context,
                v8::String::NewFromUtf8(isolate, prop.name.c_str()).ToLocalChecked(),
                userData.marshallingData->toV8(callCtx, (u8*)value + prop.offset, valueNeedsCopy)
            ).Check();
        }
    ...
    void* TrivialStructMarshaller::convertFromV8(CallContext& callCtx, const v8::Local<v8::Value>& value) {
        ...
        if (!value->IsObject()) {
            isolate->ThrowException(
                v8::Exception::TypeError(
                    v8::String::NewFromUtf8(isolate, "Value is not an object").ToLocalChecked()
                )
            );
        ...
        for (u32 i = 0; i < properties.size(); i++) {
            ...
            userData.marshallingData->fromV8(callCtx, propValue);
