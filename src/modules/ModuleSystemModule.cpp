...
namespace tspp {
    ...
    void DefineCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
        ...
        if (args.Length() > argIndex && args[argIndex]->IsArray()) {
            ...
            for (uint32_t i = 0; i < deps->Length(); i++) {
                ...
                if (deps->Get(context, i).ToLocal(&dep) && dep->IsString()) {
                    ...
                    isolate->ThrowException(v8::Exception::TypeError(
                        v8::String::NewFromUtf8(isolate, "define requires an array of strings as dependencies")
                            .ToLocalChecked()
                    ));
                    return;
                }
        ...
        if (!success) {
            isolate->ThrowException(
                v8::Exception::Error(v8::String::NewFromUtf8(isolate, "Failed to define module").ToLocalChecked())
            );
            return;
        }
    }
    ...
    void RelativeRequireCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
        ...
        v8::MaybeLocal<v8::Value> thisId =
            moduleObj->Get(context, v8::String::NewFromUtf8(isolate, "id").ToLocalChecked());
    ...
    void ModuleSystemModule::setupDefineFunction() {
        ...
        v8::Local<v8::Function> defineFunc =
            v8::Function::New(context, DefineCallback, v8::External::New(isolate, this)).ToLocalChecked();

        // Add the define.amd property (for AMD compliance)
        defineFunc->Set(context, v8::String::NewFromUtf8(isolate, "amd").ToLocalChecked(), v8::Object::New(isolate))
            .Check();
        ...
        global->Set(context, v8::String::NewFromUtf8(isolate, "define").ToLocalChecked(), defineFunc).Check();
    }
    ...
    void ModuleSystemModule::setupRequireFunction() {
        ...
        v8::Local<v8::Function> requireFunc =
            v8::Function::New(context, RequireCallback, v8::External::New(isolate, this)).ToLocalChecked();
        ...
        // Add it to the global object
        v8::Local<v8::Object> global = context->Global();
        global->Set(context, v8::String::NewFromUtf8(isolate, "require").ToLocalChecked(), requireFunc).Check();
    }
    ...
    bool ModuleSystemModule::registerBuiltInModule(const String& id, v8::Local<v8::Object> moduleObject) {
        ...
        if (m_modules.find(id) != m_modules.end()) {
            warn("Module '%s' is already registered", id.c_str());
            return false;
        }
        ...
        debug("Registered built-in module: %s", id.c_str());
        return true;
    }

    bool ModuleSystemModule::defineModule(
        const String& id, const Array<String>& deps, v8::Local<v8::Function> factory
    ) {
        v8::Isolate* isolate = m_scriptSystem->getIsolate();
        v8::HandleScope scope(isolate);
        v8::Local<v8::Context> context = m_scriptSystem->getContext();
        ...
        if (m_modules.find(moduleId) != m_modules.end()) {
            warn("Module '%s' is already defined", moduleId.c_str());
            return false;
        }
        ...
        debug("Defined module: %s", moduleId.c_str());
        return true;
    }
    ...
    v8::Local<v8::Value> ModuleSystemModule::loadModule(const String& id) {
        ...
        if (it == m_modules.end()) {
            // Module not found
            error("Module '%s' not found", id.c_str());
        ...
        moduleObj->Set(context, v8::String::NewFromUtf8(isolate, "exports").ToLocalChecked(), exportsObj).Check();
        ...
        bool success = factory->Call(context, v8::Undefined(isolate), args.size(), argArray).ToLocal(&result);
        ...
        if (!success) {
            error("Error executing factory function for module '%s'", id.c_str());
        ...
        if (!result->IsUndefined()) {
            ...
            exportsObj = v8::Local<v8::Object>::Cast(
                moduleObj->Get(context, v8::String::NewFromUtf8(isolate, "exports").ToLocalChecked()).ToLocalChecked()
            );
        }
    ...
    void ModuleSystemModule::createModuleArgs(
        ...
        requireFunc = v8::Function::New(context, RelativeRequireCallback, data).ToLocalChecked();
    }
}